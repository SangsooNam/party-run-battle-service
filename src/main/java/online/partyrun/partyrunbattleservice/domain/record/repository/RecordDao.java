...
public class RecordDao {
    ...
    public Optional<Record> findLatestRunnerRecord(String battleId, String runnerId) {
        Aggregation aggregation = Aggregation.newAggregation(RunnerRecord.class,
        ...
        AggregationResults<Record> results = mongoTemplate.aggregate(aggregation, Battle.class, Record.class);
    ...
    private ProjectionOperation getRecordProjectionOperation() {
        return project()
                .and("records.location.point").as("location.point")
                .and("records.location.altitude").as("location.altitude")
                .and("records.time").as("time")
                .and("records.distance").as("distance")
                .andExclude("_id");
    }

    public void pushNewRecords(String battleId, String runnerId, List<Record> records) {
        Query query = Query.query(Criteria.where("battleId").is(battleId).and("runnerId").is(runnerId));
